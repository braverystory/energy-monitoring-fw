...
void handleRoot() {
  ...
  server.send(200, "text/html", myhtmlPage.c_str()); 
  printf("The user visited the home page\r\n");
  
}
void handleRTCPage() {      
    std::string rtcPage = std::string("") + 
    "<html>" + 
    "<head>" + 
    "    <meta charset=\"utf-8\">" + 
    "    <title>ESP32-S3-Relay-1CH</title>" + 
    ...
    server.send(200, "text/html", rtcPage.c_str());   
...
void handleGetData() {
  ...
  for (int i = 0; i < Relay_Number_MAX; i++) {
    json += std::to_string(Relay_Flag[i]);
...
void handleNewEvent(void) {
  ...
  if (server.hasArg("data")) {
    std::string newData = server.arg("data").c_str();
    std::copy(newData.begin(), newData.end(), Text);
    Text[newData.length()] = '\0';
  }
  server.send(200, "text/plain", "OK");
...
void handleUpTimeAndEvent() {
  ...
  jsonResponse += "\"time\":\"" + std::string(datetime_str) + "\",";
  for (int i = 0; i < eventCount; i++) {
    jsonResponse += "\"eventStr" + std::to_string(i + 1) + "\":\"" + std::string(Event_str[i]) + "\",";
  }
  jsonResponse += "\"eventCount\":" + std::to_string(eventCount);
  jsonResponse += "}";
  // Send the datetime string as a response
  server.send(200, "text/plain", jsonResponse.c_str());
}
void handleDeleteEvent() {
  if (server.hasArg("id")) {
    ...
    if (id > 0) {
      ...
      server.send(200, "text/plain", ("Event " + std::to_string(id) + " deleted.").c_str());
