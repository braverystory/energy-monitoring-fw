...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr.find('/') != std::string::npos) {
    // The topic includes a '/', we'll try to read the number of bottles from just after that
    topicStr.erase(0, topicStr.find('/')+1);
    // Now see if there's a number of bottles after the '/'
    bottleCount = std::stoi(topicStr);
  }
  ...
  if (bottleCount > 0) {
    ...
    for (int i = bottleCount; i > 0; i--) {
      std::string numBottles = std::to_string(i);
      msgLen += 2*numBottles.length();
      if (i == 1) {
        msgLen += 2*std::string(" green bottle, standing on the wall\n").length();
      } else {
        msgLen += 2*std::string(" green bottles, standing on the wall\n").length();
      }
      msgLen += std::string("And if one green bottle should accidentally fall\nThere'll be ").length();
      switch (i) {
      case 1:
        msgLen += std::string("no green bottles, standing on the wall\n\n").length();
        break;
      case 2:
        msgLen += std::string("1 green bottle, standing on the wall\n\n").length();
        break;
      default:
        numBottles = std::to_string(i-1);
        msgLen += numBottles.length();
        msgLen += std::string(" green bottles, standing on the wall\n\n").length();
        break;
      };
    }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    clientId += std::to_string(random(0xffff));
    // Attempt to connect
